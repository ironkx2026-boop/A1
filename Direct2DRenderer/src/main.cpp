...
static Options parse_args(int argc, wchar_t** argv) {
    ...
    if (options.input.empty() || options.output.empty() || options.prefix.empty()) {
        throw std::runtime_error("Required arguments: --input <pdf> --output <folder> --dpi <value> --prefix <prefix>");
    }
...
static std::wstring page_path(const Options& options, uint32_t pageIndex) {
    wchar_t number[16]{};
    swprintf_s(number, L"%04u", pageIndex + 1);
    return (fs::path(options.output) / (options.prefix + number + L".png")).wstring();
}
...
static void save_texture_png(RenderDevice& device, ID3D11Texture2D* texture, const std::wstring& outputPath) {
    ...
    try {
        check(device.wicFactory->CreateStream(&stream), "Create WIC stream failed");
        check(stream->InitializeFromFilename(outputPath.c_str(), GENERIC_WRITE), "Open PNG output failed");
        check(device.wicFactory->CreateEncoder(GUID_ContainerFormatPng, nullptr, &encoder), "Create PNG encoder failed");
        check(encoder->Initialize(stream.Get(), WICBitmapEncoderNoCache), "Initialize PNG encoder failed");
        check(encoder->CreateNewFrame(&frame, &properties), "Create PNG frame failed");
        check(frame->Initialize(properties.Get()), "Initialize PNG frame failed");
        check(frame->SetSize(desc.Width, desc.Height), "Set PNG size failed");

        WICPixelFormatGUID format = GUID_WICPixelFormat32bppBGRA;
        check(frame->SetPixelFormat(&format), "Set PNG pixel format failed");
        check(frame->WritePixels(desc.Height, mapped.RowPitch, mapped.RowPitch * desc.Height,
            static_cast<BYTE*>(mapped.pData)), "Write PNG pixels failed");
        check(frame->Commit(), "Commit PNG frame failed");
        check(encoder->Commit(), "Commit PNG encoder failed");
    } catch (...) {
...
static void render_page(RenderDevice& device, pdf::PdfPage const& page, const Options& options, uint32_t pageIndex) {
    ...
    const auto outputPath = page_path(options, pageIndex);
    save_texture_png(device, texture.Get(), outputPath);
    std::wcout << L"page " << (pageIndex + 1) << L" -> " << outputPath << std::endl;
}
...
int wmain(int argc, wchar_t** argv) {
    try {
        ...
        for (uint32_t pageIndex = 0; pageIndex < pageCount; ++pageIndex) {
            ...
            render_page(device, page, options, pageIndex);
            page.Close();
        }

        std::wcout << L"done " << pageCount << L" page(s)" << std::endl;
        return 0;
